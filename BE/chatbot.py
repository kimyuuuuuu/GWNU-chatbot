...
def get_result_streaming_sync(chain: Runnable, question: str, context: List[Document]):
    return chain.stream({"question": question, "context": context})


def get_result_streaming_async(chain: Runnable, question: str, context: List[Document]):
    return chain.astream({"question": question, "context": context})
...
def process_question_streaming(is_sync: bool, user_question: str):
    embeddings = get_embeddings()
    ...
    if is_sync:
        response = get_result_streaming_sync(chain, user_question, retrieve_docs)
    else:
        response = get_result_streaming_async(chain, user_question, retrieve_docs)
